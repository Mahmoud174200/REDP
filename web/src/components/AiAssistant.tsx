...
import { MessageCircle, Send, X, Mic, Volume2, VolumeX, Trash2, Phone, PhoneOff } from 'lucide-react';
...
const AiAssistant: React.FC<Props> = ({ mode }) => {
  ...
  const send = useCallback(
    ...
    [input, loading, endpoint, speak, lang]
  );
    ...
    return (
      <button
        ...
        style={{
          ...
          left: '24px',
          zIndex: 9998,
          width: isCustomer ? '116px' : '62px',
          height: isCustomer ? '116px' : '62px',
          borderRadius: isCustomer ? '0' : '50%',
          border: 'none',
          cursor: 'pointer',
          overflow: 'visible',
          padding: 0,
          background: isCustomer ? 'transparent' : `linear-gradient(135deg, ${ACCENT}, ${ACCENT2})`,
          color: '#fff',
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'center',
          boxShadow: isCustomer ? 'none' : '0 10px 30px rgba(99,102,241,0.45)',
        }}
      >
        {isCustomer ? <NourAvatar state="idle" size={116} /> : <MessageCircle style={{ width: 26, height: 26 }} />}
      </button>
    );
  }
